...
!function (e) {
    ...
    var i = function (e) {
        ...
        data: {}, init: function (e) {
            ...
            i.length > 0 && !M(i) && (i = i.split(e.initialPreviewDelimiter)), a.data[t] = {
                content: i,
            ...
            var r = n.config[i], l = N("caption", r) ? r.caption : "", o = N("width", r) ? r.width : "auto", s = N("url", r) ? r.url : !1, d = N("key", r) ? r.key : null, c = s === !1 && t, p = n.isDelete ? n.actions(!1, !0, c, s, d) : "", u = n.footer.repl("{actions}", p);
    ...
    }, _ = window.URL || window.webkitURL, q = function (t, a) {
        var n = this;
    ...
    q.prototype = {
        constructor: q, validate: function () {
            var e, i = this;
            ...
            return z(i.customLayoutTags) ? t : W(t, i.customLayoutTags)
        }, getPreviewTemplate: function (e) {
            var i = this, t = N(e, i.previewTemplates) ? i.previewTemplates[e] : A[e];
            return t = t.repl("{previewFileIcon}", i.previewFileIcon), z(i.customPreviewTags) ? t : W(t, i.customPreviewTags)
            ...
            t(a, "change", e.proxy(i.change, i)), t(e(window), "resize", function () {
                i.setEllipsis()
            }), t(r, "click", function () {
                i.raise("filebrowse"), i.isError && !i.isUploadable && i.clear(), n.focus()
            }), t(l, "reset", e.proxy(i.reset, i)), t(o.find(".fileinput-remove:not([disabled])"), "click", e.proxy(i.clear, i)), t(o.find(".fileinput-cancel"), "click", e.proxy(i.cancel, i)), i.isUploadable && i.dropZoneEnabled && i.showPreview && i.initDragDrop(), i.isUploadable || t(l, "submit", e.proxy(i.submitForm, i)), t(i.$container.find(".kv-fileinput-upload"), "click", function (t) {
            ...
            z(a) || i.$progress.html(a.repl("{percent}", t))
        }, upload: function () {
            ...
            i.getThumbs().each(function () {
                ...
                -1 !== r && (void 0 !== l ? (a[t] = l, n.attr({
                    ...
                    "data-fileindex": t
                }), t += 1) : n.attr({id: "uploaded-" + Z(), "data-fileindex": "-1"}))
            }), i.filestack = a
        }, destroy: function () {
            ...
            i.$preview.find(".kv-file-remove").each(function () {
                ...
                t(l, "click", function () {
                    i.cleanMemory(o), o.fadeOut("slow", function () {
            ...
            return n.isUploadable ? (t = l.repl("{actions}", n.renderFileActions(!0, !0, !1, !1, !1)), a = t.repl("{caption}", e).repl("{width}", i).repl("{indicator}", r.indicatorNew).repl("{indicatorTitle}", r.indicatorNewTitle)) : a = l.repl("{actions}", "").repl("{caption}", e).repl("{width}", i).repl("{indicator}", "").repl("{indicatorTitle}", ""), a = W(a, n.previewThumbTags)
            ...
            i.$preview.find(".kv-file-remove").each(function () {
                ...
                if (!z(o) && void 0 !== d) {
                    var c, p, u, f, v = l.closest(".file-preview-frame"), h = a.data[i.id], m = v.data("fileindex");
                    m = parseInt(m.replace("init_", "")), u = z(h.config) && z(h.config[m]) ? null : h.config[m], f = z(u) || z(u.extra) ? n : u.extra, "function" == typeof f && (f = f()), p = {
                        id: l.attr("id"),
                        key: d,
                        extra: f
                    }, c = e.extend({
            ...
            var e = this;
            e.resetPreview(), e.setEllipsis(), e.$container.find(".fileinput-filename").text(""), e.raise("filereset"), s(e.$container, "file-input-new"), (e.$preview.find(".file-preview-frame").length || e.isUploadable && e.dropZoneEnabled) && e.$container.removeClass("file-input-new"), e.setFileDropZoneTitle(), e.filestack = [], e.formdata = {}
        }, disable: function () {
            var e = this;
            e.isDisabled = !0, e.raise("filedisabled"), e.$element.attr("disabled", "disabled"), e.$container.find(".kv-fileinput-caption").addClass("file-caption-disabled"), e.$container.find(".btn-file, .fileinput-remove, .kv-fileinput-upload, .file-preview-frame button").attr("disabled", !0), e.initDragDrop()
        }, enable: function () {
            var e = this;
            e.isDisabled = !1, e.raise("fileenabled"), e.$element.removeAttr("disabled"), e.$container.find(".kv-fileinput-caption").removeClass("file-caption-disabled"), e.$container.find(".btn-file, .fileinput-remove, .kv-fileinput-upload, .file-preview-frame button").removeAttr("disabled"), e.initDragDrop()
            ...
            var r, l, o, d, c, p, u, f, v, h = this, m = h.getFileStack().length, g = new FormData, w = h.previewInitId + "-" + i, b = e("#" + w + ":not(.file-preview-initial)"), x = b.find(".kv-file-upload"), C = b.find(".kv-file-remove"), y = h.filestack.length > 0 || !e.isEmptyObject(h.uploadExtraData), T = {
                id: w,
                index: i
            };
            h.formdata = g, 0 === m || !y || x.hasClass("disabled") || h.abort(T) || (o = function () {
                var e = h.getThumbs(".file-uploading");
                e.length > 0 || h.fileBatchCompleted || (h.fileBatchCompleted = !0, setTimeout(function () {
                    a.set(h.id, h.uploadCache.content, h.uploadCache.config, h.uploadCache.tags, h.uploadCache.append), h.hasInitData && (h.initPreview(), h.initPreviewDeletes()), h.setProgress(100), h.unlock(), h.clearFileInput(), h.raise("filebatchuploadcomplete", [h.filestack, h.getExtraData()])
                }, 100))
            }, d = function () {
                ...
                r = h.getOutData(t), b.hasClass("file-preview-success") || (h.setThumbStatus(b, "Loading"), s(b, "file-uploading")), x.attr("disabled", !0), C.attr("disabled", !0), n || h.lock(), h.raise("filepreupload", [r, w, i]), T = e.extend(T, r), h.abort(T) && (t.abort(), h.setProgress(100))
            }, u = function (t, a, l) {
                r = h.getOutData(l, t), T = e.extend(T, r), setTimeout(function () {
                    z(t) || z(t.error) ? (h.setThumbStatus(b, "Success"), x.hide(), h.filestack[i] = void 0, h.raise("fileuploaded", [r, w, i]), h.initUploadSuccess(t, b, n), n || h.resetFileStack()) : (h.setThumbStatus(b, "Error"), h.showUploadError(t.error, T))
                }, 100)
            }, f = function () {
                setTimeout(function () {
                    d(), c(), n ? o() : h.unlock(!1), h.initSuccessThumbs()
                }, 100)
            }, v = function (a, r, l) {
                var o = h.parseError(a, l, n ? t[i].name : null);
                h.setThumbStatus(b, "Error"), T = e.extend(T, h.getOutData(a)), h.showUploadError(o, T)
            }, g.append(h.uploadFileAttr, t[i]), g.append("file_id", i), h.ajaxSubmit(p, u, f, v, w, i))
        }, uploadBatch: function () {
            var i, t, a, n, r, l = this, o = l.filestack, d = o.length, c = l.filestack.length > 0 || !e.isEmptyObject(l.uploadExtraData), p = {};
            ...
            var i, t, a, n, r = this;
            for (n = 0; n < L.length; n += 1)if (a = L[n], i = N(a, r.fileTypeSettings) ? r.fileTypeSettings[a] : R[a], t = i(e.type, e.name) ? a : "", !z(t))return t;
            return "other"
        }, previewDefault: function (i, t, a) {
            if (this.showPreview) {
                var n = this, r = _.createObjectURL(i), l = (e("#" + t), n.previewSettings.other || O.other), o = n.renderFileFooter(i.name, l.width), s = n.getPreviewTemplate("other"), d = t.slice(t.lastIndexOf("-") + 1), c = "";
            ...
            function t(e) {
                ...
                var m, g, w, b, x, C, y = v + e, T = p + "-" + y, E = i[e], F = a.slug(E.name), $ = (E.size || 0) / 1e3, k = "", I = _.createObjectURL(E), P = 0, D = a.allowedFileTypes, S = z(D) ? "" : D.join(", "), U = a.allowedFileExtensions, j = z(U) ? "" : U.join(", ");
                if (z(U) || (k = new RegExp("\\.(" + U.join("|") + ")$", "i")), $ = $.toFixed(2), a.maxFileSize > 0 && $ > a.maxFileSize)return b = a.msgSizeTooLarge.replace("{name}", F).replace("{size}", $).replace("{maxSize}", a.maxFileSize), void(a.isError = h(b, E, T, e));
                if (!z(D) && M(D)) {
                    for (w = 0; w < D.length; w += 1)x = D[w], g = f[x], C = void 0 !== g && g(E.type, F), P += z(C) ? 0 : C.length;
                    if (0 === P)return b = a.msgInvalidFileType.replace("{name}", F).replace("{types}", S), void(a.isError = h(b, E, T, e))
                }
                return 0 !== P || z(U) || !M(U) || z(k) || (C = F.match(k), P += z(C) ? 0 : C.length, 0 !== P) ? a.showPreview ? (r.length > 0 && void 0 !== FileReader ? (s.html(d.replace("{index}", e + 1).replace("{files}", u)), o.addClass("loading"), l.onerror = function (e) {
                    a.errorHandler(e, F)
                }, l.onload = function (i) {
                    a.previewFile(e, E, i, T, I), a.initFileActions()
                }, l.onloadend = function () {
                    b = c.replace("{index}", e + 1).replace("{files}", u).replace("{percent}", 50).replace("{name}", F), setTimeout(function () {
                    ...
                    if (i.lengthComputable) {
                        ...
                        b = c.replace("{index}", e + 1).replace("{files}", u).replace("{percent}", a).replace("{name}", F), setTimeout(function () {
                            s.html(b)
                        }, 100)
                    }
                }, m = N("text", f) ? f.text : R.text, m(E.type, F) ? l.readAsText(E, a.textEncoding) : l.readAsArrayBuffer(E)) : (a.previewDefault(E, T), setTimeout(function () {
                    t(e + 1), a.updateFileDetails(u)
                }, 100), a.raise("fileloaded", [E, T, e, l])), void a.filestack.push(E)) : (a.filestack.push(E), setTimeout(t(e + 1), 100), void a.raise("fileloaded", [E, T, e, l])) : (b = a.msgInvalidFileExtension.replace("{name}", F).replace("{extensions}", j), void(a.isError = h(b, E, T, e)))
            }
            ...
            var r, l, o, s, d, c, p = (t.$preview, arguments.length > 1), u = p ? i.originalEvent.dataTransfer.files : n.get(0).files, f = z(n.attr("multiple")), v = 0, h = 0, m = t.filestack.length, g = t.isUploadable, w = f && m > 0, b = function (i, a, n, r) {
            ...
            if (t.reader = null, t.resetUpload(), t.hideFileIcon(), t.isUploadable && t.$container.find(".file-drop-zone ." + t.dropZoneTitleClass).remove(), p)for (r = []; u[v];)s = u[v], s.type || s.size % 4096 !== 0 ? r.push(s) : h++, v++; else r = void 0 === i.target.files ? i.target && i.target.value ? [{name: i.target.value.replace(/^.+\\/, "")}] : [] : i.target.files;
            if (z(r) || 0 === r.length)return g || t.clear(), t.showFolderError(h), void t.raise("fileselectnone");
            if (t.resetErrors(), c = r.length, o = t.isUploadable ? t.getFileStack().length + c : c, t.maxFileCount > 0 && o > t.maxFileCount) {
                if (!t.autoReplace || c > t.maxFileCount)return d = t.autoReplace && c > t.maxFileCount ? c : o, l = t.msgFilesTooMany.replace("{m}", t.maxFileCount).replace("{n}", d), t.isError = b(l, null, null, null), t.$captionContainer.find(".kv-caption-icon").hide(), t.setCaption("", !0), t.setEllipsis(), void t.$container.removeClass("file-input-new file-input-ajax-new");
                o > t.maxFileCount && t.resetPreviewThumbs(g)
            } else!g || w ? (t.resetPreviewThumbs(!1), w && (t.filestack = [])) : !g || 0 !== m || a.count(t.id) && !t.overwriteInitial || t.resetPreviewThumbs(!0);
            t.isPreviewable ? t.readFiles(r) : t.updateFileDetails(1), t.showFolderError(h)
        }, validateImage: function (e, i) {
            var a, n, r, l, o = this, s = o.$preview, d = s.find("#" + i), c = "Untitled", p = d.find("img");
            p.length && t(p, "load", function () {
                n = d.width(), r = s.width(), n > r && (p.css("width", "100%"), d.css("width", "97%")), l = p.closest(".file-preview-frame").find(".file-caption-name"), l.length && (l.width(p.width()), c = l.text(), l.attr("title", c)), a = {
                    ind: e,
                    id: i
                }, o.checkDimensions(e, "Small", p, d, c, "Width", a), o.checkDimensions(e, "Small", p, d, c, "Height", a), o.checkDimensions(e, "Large", p, d, c, "Width", a), o.checkDimensions(e, "Large", p, d, c, "Height", a), o.raise("fileimageloaded", [i]), _.revokeObjectURL(p.attr("src"))
            })
        ...
        if (r() || i(9)) {
            ...
            return a.shift(), this.each(function () {
                ...
                r || (i = e.extend({}, e.fn.fileinput.defaults), "en" === o || z(e.fn.fileinputLocales[o]) || (i = e.extend(i, e.fn.fileinputLocales[o])), r = new q(this, e.extend(i, l, n.data())), n.data("fileinput", r)), "string" == typeof t && r[t].apply(r, a)
            })
        }
        ...
        msgFilePreviewError: '在读取文件的时候出现错误 "{name}".',
        msgInvalidFileType: '无效的文件类型 "{name}". 仅仅支持这些"{types}" 格式是被支持的.',
        msgInvalidFileExtension: '不正确的文件扩展名 "{name}". Only "{extensions}" files are supported.',
        msgValidationError: "上传文件出错",
        msgLoading: "加载到第{index}个{files} &hellip;",
        msgProgress: "加载第{index}个{files}文件 - {name} -完成{percent}% .",
        msgSelected: "{n} {files} selected",
        msgFoldersNotAllowed: "仅仅支持拖动文件到此! {n} 文件夹会被自动忽略.",
        msgImageWidthSmall: '"{name}"图片文件宽度至少为{size} px.',
        msgImageHeightSmall: '"{name}"图片文件高度至少为{size} px.',
        msgImageWidthLarge: '"{name}"图片文件宽度不能够超过 {size} px.',
        msgImageHeightLarge: '"{name}"图片文件高度不能够超过 {size} px.',
        dropZoneTitle: "请拖拽文件到此 &hellip;"
    }, e.extend(e.fn.fileinput.defaults, e.fn.fileinputLocales.en), e.fn.fileinput.Constructor = q, e(document).ready(function () {
